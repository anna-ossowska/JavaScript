...
function convertToRoman(num) {
  ...
  // create an array of decimal numbers' indexes passing the condition
  for (let i = 0; i < decimal.length; i++) {
    while (decimal[i] <= num) {
      searchDecimal.push(decimal.indexOf(decimal[i]));
      num -= decimal[i];
    }
  }

  // Convert indexes to its Roman counterparts and join
  searchDecimal.map((item) => {
    searchRoman.push(roman[item]);
  });
  return searchRoman.join('');
}
